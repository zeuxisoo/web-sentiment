    ...
    public function store() {
        ...
        if ($validator->fails()) {
            ...
            if ($answer_a_image) {
                ...
                $answer_file_path = $attachment_path.'/answer_image/a/'.$fileName;
                $cover_file_path  = $attachment_path.'/cover_image/a/'.$fileName;
                ...
                $answer_a_image = Image::canvas(530, 530, '#FFFFFF')->insert(
                    ...
                    'center'
                )->save($answer_file_path, 100);

                $cover_a_image = Image::canvas(64, 64, '#FFFFFF')->insert(
                    Image::make($answer_a_image)->resize(64, null, function ($constraint) {
                        $constraint->aspectRatio();
                        $constraint->upsize();
                    }),
                    'center'
                )->save($cover_file_path, 100);
            }
            ...
            if ($answer_b_image) {
                ...
                $answer_file_path  = $attachment_path.'/answer_image/b/'.$fileName;
                $cover_file_path  = $attachment_path.'/cover_image/b/'.$fileName;
                ...
                $cover_b_image = Image::canvas(64, 64, '#FFFFFF')->insert(
                    Image::make($answer_b_image)->resize(64, null, function ($constraint) {
                        $constraint->aspectRatio();
                        $constraint->upsize();
                    }),
                    'center'
        ...
        if ($topic->user_id !== Auth::user()->id) {
            ...
            if ($validator->fails()) {
                ...
                if ($answer_a_image) {
                    ...
                    if (File::exists($topic->coverAImagePath()) === true) {
                        File::delete($topic->coverAImagePath());
                    }
                }
                ...
                if ($answer_b_image) {
                    ...
                    if (File::exists($topic->coverBImagePath()) === true) {
                        File::delete($topic->coverBImagePath());
                    }
                }
